...
resource "azurerm_logic_app_action_custom" "logic_app_cosmosdb_createorupdatedocumentv3" {
  ...
  logic_app_id = format("%s/%s", "/subscriptions/${var.ARM_SUBSCRIPTION_ID}/providers/Microsoft.Logic/locations/${var.RG_LOCATION}/providers/Microsoft.Logic/workflows/", azurerm_logic_app_workflow.logic_app_workflow.name)

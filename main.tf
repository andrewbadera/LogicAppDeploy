...
resource "azurerm_logic_app_workflow" "logic_app_workflow" {
  location = "${var.RG_LOCATION}"
  name     = "la-ghdeploy-${var.LOCATION_ABBREVIATION}-${github.env}"
  parameters = {
...
resource "azurerm_logic_app_action_custom" "logic_app_cosmosdb_createorupdatedocumentv3" {
  ...
  logic_app_id = "/subscriptions/${var.ARM_SUBSCRIPTION_ID}/resourceGroups/${var.RG_NAME}/providers/Microsoft.Logic/workflows/la-ghdeploy-${var.LOCATION_ABBREVIATION}-${github.env}"
